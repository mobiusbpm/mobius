 ...
 */
package mobius.app.engine.test.persistence;
...
public class EntitiesTest {
    ...
    protected Class getAndAssertEntityInterfaceClass(String mappedResource) {
        try {
            Class c = Class.forName("mobius.engine.impl.persistence.entity." + mappedResource + "Entity");
            assertNotNull(c);
            return c;
        } catch (Exception e) {
    ...
    protected Class getAndAssertEntityImplClass(String mappedResource) {
        try {
            Class c = Class.forName("mobius.engine.impl.persistence.entity." + mappedResource + "EntityImpl");
            assertNotNull(c);
            return c;
        } catch (Exception e) {
            throw new AssertionError("Entity interface class for " + mappedResource + " not found", e);
    ...
    private Set<String> getMappedResources() {
        try {
            ...
            for (int i = 0; i < nodeList.getLength(); i++) {
                ...
                if (resource.startsWith("mobius/app") && !resource.contains("common.xml")) {
                    resource = resource.replaceAll("mobius/app/db/mapping/entity/", "");
