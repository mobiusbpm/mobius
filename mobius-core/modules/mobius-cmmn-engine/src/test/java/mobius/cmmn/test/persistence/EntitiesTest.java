 ...
 */
package mobius.cmmn.test.persistence;

import mobius.cmmn.engine.CmmnEngineConfiguration;
import mobius.cmmn.engine.impl.db.EntityDependencyOrder;
import mobius.cmmn.engine.impl.persistence.entity.data.impl.TableDataManagerImpl;
...
public class EntitiesTest {
    ...
    protected Class getAndAssertEntityInterfaceClass(String mappedResource) {
        try {
            Class c = Class.forName("mobius.cmmn.engine.impl.entity." + mappedResource + "Entity");
            assertNotNull(c);
            return c;
        } catch (Exception e) {
    ...
    protected Class getAndAssertEntityImplClass(String mappedResource) {
        try {
            Class c = Class.forName("mobius.cmmn.engine.impl.entity." + mappedResource + "EntityImpl");
            assertNotNull(c);
            return c;
        } catch (Exception e) {
            throw new AssertionError("Entity interface class for " + mappedResource + " not found", e);
    ...
    private Set<String> getMappedResources() {
        try {
            ...
            for (int i = 0; i < nodeList.getLength(); i++) {
                ...
                if (resource.startsWith("mobius/cmmn") && !resource.contains("common.xml")) {
                    resource = resource.replaceAll("mobius/cmmn/db/mapping/entity/", "");
