...
import mobius.engine.impl.event.logger.handler.Fields;
...
public class DatabaseEventLoggerTest extends PluggableFlowableTestCase {
    ...
    @Test
    @Deployment(resources = { "mobius/engine/test/api/event/DatabaseEventLoggerProcess.bpmn20.xml" })
    public void testDatabaseEvents() throws IOException {

        String testTenant = "testTenant";

        String deploymentId = repositoryService.createDeployment()
                .addClasspathResource("mobius/engine/test/api/event/DatabaseEventLoggerProcess.bpmn20.xml")
                .tenantId(testTenant)
                .deploy().getId();
        ...
        for (int i = 0; i < eventLogEntries.size(); i++) {
            ...
            // mobius.task.service.Task completion
            if (i == 1 || i == 6) {
                assertNotNull(entry.getType());
    ...
    public void testDatabaseEventsNoTenant() throws IOException {
        ...
        String deploymentId = repositoryService.createDeployment().addClasspathResource("mobius/engine/test/api/event/DatabaseEventLoggerProcess.bpmn20.xml").deploy().getId();
