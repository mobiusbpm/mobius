...
public class JobEventsTest extends PluggableFlowableTestCase {
    ...
    @Test
    @Deployment(resources = "mobius/engine/test/api/event/JobEventsTest.testJobCanceledEventOnBoundaryEvent.bpmn20.xml")
    ...
    public void testJobCanceledAndTimerStartEventOnProcessRedeploy() throws Exception {
        ...
        String deployment1 = repositoryService.createDeployment().addClasspathResource("mobius/engine/test/api/event/JobEventsTest.testTimerFiredForTimerStart.bpmn20.xml").deploy().getId();
        checkEventCount(1, FlowableEngineEventType.TIMER_SCHEDULED);
        listener.clearEventsReceived();

        // WHEN
        String deployment2 = repositoryService.createDeployment().addClasspathResource("mobius/engine/test/api/event/JobEventsTest.testTimerFiredForTimerStart.bpmn20.xml").deploy().getId();
