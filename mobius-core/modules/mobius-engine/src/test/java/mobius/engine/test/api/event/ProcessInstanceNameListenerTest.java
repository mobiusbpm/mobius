...
public class ProcessInstanceNameListenerTest extends PluggableFlowableTestCase {
    ...
    public void testCallActivityProcessCreatedDefinitionName() throws Exception {
        BpmnModel mainBpmnModel = loadBPMNModel("mobius/engine/test/bpmn/subprocess/SubProcessTest.testSuspendedProcessCallActivity_mainProcess.bpmn.xml");
        BpmnModel childBpmnModel = loadBPMNModel("mobius/engine/test/bpmn/subprocess/SubProcessTest.testSuspendedProcessCallActivity_childProcess.bpmn.xml");

        mobius.engine.repository.Deployment childDeployment = processEngine.getRepositoryService().createDeployment().name("childProcessDeployment").addBpmnModel("childProcess.bpmn20.xml", childBpmnModel).deploy();

        mobius.engine.repository.Deployment masterDeployment = processEngine.getRepositoryService().createDeployment().name("masterProcessDeployment").addBpmnModel("masterProcess.bpmn20.xml", mainBpmnModel).deploy();
