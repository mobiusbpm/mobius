...
public class TaskEventsTest extends PluggableFlowableTestCase {
     ...
     */
    @Test
    @Deployment(resources = { "mobius/engine/test/api/event/TaskEventsTest.testEventFiring.bpmn20.xml" })
    public void testEventFiringOrdering() {
        // We need to add a special listener that copies the mobius.task.service.Task values - to record its state when the event fires,
