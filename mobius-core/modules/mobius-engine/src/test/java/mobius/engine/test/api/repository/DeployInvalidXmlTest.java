...
public class DeployInvalidXmlTest extends PluggableFlowableTestCase {
    ...
    public void testDeployNonSchemaConformantXml() {
        try {
            repositoryService.createDeployment().addClasspathResource("mobius/engine/test/api/repository/nonSchemaConformantXml.bpmn20.xml").deploy().getId();
            fail();
    ...
    public void testDeployWithMissingWaypointsForSequenceflowInDiagramInterchange() {
        try {
            repositoryService.createDeployment().addClasspathResource("mobius/engine/test/api/repository/noWayPointsForSequenceFlowInDiagramInterchange.bpmn20.xml").deploy().getId();
            fail();
        } catch (XMLException e) {
            // expected exception
        }
    ...
    private static String UNSAFE_XML = "<?xml version='1.0' encoding='UTF-8'?>" + "<!-- Billion Laugh attacks : http://portal.sliderocket.com/CJAKM/xml-attacks -->" + "<!DOCTYPE lols ["
            ...
            + "xmlns:activiti='http://activiti.org/bpmn'" + "targetNamespace='Examples'>" + "<process id='oneTaskProcess' name='The One mobius.task.service.Task Process'>"
    ...
    public void testExternalEntityResolvingTest() {
        String deploymentId = repositoryService.createDeployment()
                .addClasspathResource("mobius/engine/test/api/repository/DeployInvalidXmlTest.testExternalEntityResolvingTest.bpmn20.xml")
                .deploy()
                .getId();
