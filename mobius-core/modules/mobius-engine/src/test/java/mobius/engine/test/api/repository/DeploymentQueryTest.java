...
public class DeploymentQueryTest extends PluggableFlowableTestCase {
    ...
    protected void setUp() throws Exception {
        deploymentOneId = repositoryService.createDeployment().name("mobius/engine/test/repository/one.bpmn20.xml").category("testCategory")
                .addClasspathResource("mobius/engine/test/repository/one.bpmn20.xml").deploy().getId();

        deploymentTwoId = repositoryService.createDeployment().name("mobius/engine/test/repository/two.bpmn20.xml").addClasspathResource("mobius/engine/test/repository/two.bpmn20.xml")
                .deploy().getId();
    ...
    public void testQueryByName() {
        DeploymentQuery query = repositoryService.createDeploymentQuery().deploymentName("mobius/engine/test/repository/two.bpmn20.xml");
        assertNotNull(query.singleResult());
        assertEquals(1, query.list().size());
        assertEquals(1, query.count());
    }
    ...
    public void testQueryByProcessDefinitionKeyLikeMultipleProcessDefinitions() {
        String deploymentId = repositoryService.createDeployment().name("mobius/engine/test/repository/one.bpmn20.xml")
                .addClasspathResource("mobius/engine/test/repository/one.bpmn20.xml")
                .addClasspathResource("mobius/engine/test/repository/two.bpmn20.xml")
                .deploy().getId();
    ...
    public void testVerifyDeploymentProperties() {
        ...
        assertEquals("mobius/engine/test/repository/one.bpmn20.xml", deploymentOne.getName());
        assertEquals(deploymentOneId, deploymentOne.getId());

        Deployment deploymentTwo = deployments.get(1);
        assertEquals("mobius/engine/test/repository/two.bpmn20.xml", deploymentTwo.getName());
        ...
        assertEquals("mobius/engine/test/repository/one.bpmn20.xml", deployments.get(0).getName());
    ...
    public void testNativeQuery() {
        ...
        assertEquals(1, repositoryService.createNativeDeploymentQuery().sql(baseQuerySql + " where NAME_ = #{name}").parameter("name", "mobius/engine/test/repository/one.bpmn20.xml").list().size());
