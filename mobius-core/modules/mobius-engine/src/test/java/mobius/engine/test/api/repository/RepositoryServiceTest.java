...
public class RepositoryServiceTest extends PluggableFlowableTestCase {
    ...
    public void testDeploymentWithDelayedProcessDefinitionActivation() {
        ...
        mobius.engine.repository.Deployment deployment = repositoryService.createDeployment().addClasspathResource("mobius/engine/test/api/oneTaskProcess.bpmn20.xml")
                .addClasspathResource("mobius/engine/test/api/twoTasksProcess.bpmn20.xml").activateProcessDefinitionsOn(inThreeDays).deploy();
    ...
    public void testGetResourceAsStreamUnexistingResourceInExistingDeployment() {
        ...
        try {
            repositoryService.getResourceAsStream(deployment.getId(), "mobius/engine/test/api/unexistingProcess.bpmn.xml");
            fail("ActivitiException expected");
        } catch (FlowableObjectNotFoundException ae) {
    ...
    public void testGetResourceAsStreamUnexistingDeployment() {
        ...
        try {
            repositoryService.getResourceAsStream("unexistingdeployment", "mobius/engine/test/api/unexistingProcess.bpmn.xml");
            fail("ActivitiException expected");
        } catch (FlowableObjectNotFoundException ae) {
            assertTextPresent("deployment does not exist", ae.getMessage());
            assertEquals(mobius.engine.repository.Deployment.class, ae.getObjectClass());
        }
    }
    ...
    public void testGetBpmnModel() {
        ...
        // Test the flow
        mobius.bpmn.model.Process process = bpmnModel.getProcesses().get(0);
    ...
    public void testDeployZipFile() {
        InputStream inputStream = this.getClass().getClassLoader().getResourceAsStream("mobius/engine/test/api/repository/test-processes.zip");
