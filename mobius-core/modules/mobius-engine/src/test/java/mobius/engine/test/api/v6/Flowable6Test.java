...
public class Flowable6Test extends PluggableFlowableTestCase {
    ...
    public void testSimplestProcessPossible() {
        repositoryService.createDeployment().addClasspathResource("mobius/engine/test/api/v6/Flowable6Test.simplestProcessPossible.bpmn20.xml").deploy();
    ...
    @Test
    @mobius.engine.test.Deployment(resources = "mobius/engine/test/api/v6/Flowable6Test.testOneTaskProcess.bpmn20.xml")
    ...
    public void testSimpleNonInterruptingTimerBoundaryEvent() {
        ...
        tasks = taskService.createTaskQuery().orderByTaskName().desc().list(); // Not the desc() here: mobius.task.service.Task B, mobius.task.service.Task A will be the result (task b being associated with the child execution)
        for (Task task : tasks) {
            taskService.complete(task.getId());
        }
        assertEquals(0, runtimeService.createExecutionQuery().count());
