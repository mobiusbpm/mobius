 ...
 */
package mobius.engine.test.bpmn.event.escalation;
...
public class EscalationEventSubProcessTest extends PluggableFlowableTestCase {
    ...
    public void testEscalationCodeTakesPrecedence() {
        ...
        // The process will throw an escalation event, which is caught and escalated by a User mobius.task.service.Task
    ...
    @Test
    @Deployment(resources = {"mobius/engine/test/bpmn/event/escalation/EscalationEventSubProcessTest.testCatchEscalationInParentSubProcess.bpmn20.xml",
                    "mobius/engine/test/bpmn/event/escalation/BoundaryEscalationEventTest.subprocess.bpmn20.xml"})
    ...
    @Test
    @Deployment(resources = {"mobius/engine/test/bpmn/event/escalation/EscalationEventSubProcessTest.testCatchChildProcessThrowEventInEventSubProcess.bpmn20.xml",
                    "mobius/engine/test/bpmn/event/escalation/throwEscalationEventSubProcess.bpmn20.xml"})
    ...
    private void assertThatEscalationHasBeenCaught(String procId) {
        ...
        // which is caught and escalated by a User mobius.task.service.Task
        assertEquals("No tasks found in task list.", 1, taskService.createTaskQuery().count());
        Task task = taskService.createTaskQuery().singleResult();
        assertEquals("Escalated Task", task.getName());

        // Completing the mobius.task.service.Task will end the process instance
        taskService.complete(task.getId());
        assertProcessEnded(procId);
    }
