...
public class DeploymentPersistenceTest extends PluggableFlowableTestCase {
    ...
    public void testDeploymentPersistence() {
        Deployment deployment = repositoryService.createDeployment().name("strings").addString("mobius/test/HelloWorld.string", "hello world").addString("mobius/test/TheAnswer.string", "42")
                .deploy();
        ...
        expectedResourceNames.add("mobius/test/HelloWorld.string");
        expectedResourceNames.add("mobius/test/TheAnswer.string");
        assertEquals(expectedResourceNames, new HashSet<>(resourceNames));

        InputStream resourceStream = repositoryService.getResourceAsStream(deploymentId, "mobius/test/HelloWorld.string");
        assertTrue(Arrays.equals("hello world".getBytes(), IoUtil.readInputStream(resourceStream, "test")));

        resourceStream = repositoryService.getResourceAsStream(deploymentId, "mobius/test/TheAnswer.string");
