...
public class AsyncExecutorTwoEnginesTest {
    ...
    private void cleanup(ProcessEngine processEngine) {
        for (mobius.engine.repository.Deployment deployment : processEngine.getRepositoryService().createDeploymentQuery().list()) {
    ...
    private String deploy(ProcessEngine processEngine, String resource) {
        return processEngine.getRepositoryService().createDeployment().addClasspathResource("mobius/engine/test/jobexecutor/" + resource).deploy().getId();
    }
