...
import mobius.common.engine.api.FlowableClassLoadingException;
...
public class JavaServiceTaskTest extends PluggableFlowableTestCase {
    ...
    public void testUnexistingClassDelegation() {
        try {
            ...
            assertTrue(e.getMessage().contains("couldn't instantiate class mobius.BogusClass"));
    ...
    public void testIllegalUseOfResultVariableName() {
        try {
            deploymentIdsForAutoCleanup.add(
                repositoryService.createDeployment().addClasspathResource("mobius/examples/bpmn/servicetask/JavaServiceTaskTest.testIllegalUseOfResultVariableName.bpmn20.xml").deploy().getId()
            );
            fail();
        } catch (FlowableException e) {
