...
public class ProcessDefinitionQueryEscapeClauseTest extends AbstractEscapeClauseTestCase {
    ...
    protected void setUp() throws Exception {
        ...
        deploymentTwoId = repositoryService
                ...
                .tenantId("Two_")
                .name("two_")
                .addClasspathResource("mobius/engine/test/repository/two_.bpmn20.xml")
                .deploy()
                .getId();
    ...
    public void testQueryByResourceNameLike() {
        ...
        assertEquals("mobius/engine/test/repository/one%.bpmn20.xml", query.singleResult().getResourceName());
        assertEquals(1, query.list().size());
        assertEquals(1, query.count());

        query = repositoryService.createProcessDefinitionQuery().processDefinitionResourceNameLike("%\\_%");
        assertEquals("mobius/engine/test/repository/two_.bpmn20.xml", query.singleResult().getResourceName());
        assertEquals(1, query.list().size());
        assertEquals(1, query.count());
    }
